...
class Decoder(nn.Module):
    ...
    def forward(self, features):
        ...
        x6_up = x6_up + x3
        # print('x6_up.shape = ',x6_up.shape)
        
        x7 = self.block_seven(x6_up)
        x7_up = self.block_seven_up(x7)
        x7_up = x7_up + x2
        # print('x7_up.shape = ',x7_up.shape)

        x8 = self.block_eight(x7_up)
        x8_up = self.block_eight_up(x8)
        x8_up = x8_up + x1
        # print('x8_up.shape = ',x8_up.shape)
        ...
        if self.has_dropout:
            x9 = self.dropout(x9)
        out_seg_1 = self.out_conv(x8_up)
        out_seg = self.out_conv(x9)
        # print('out_seg.shape = ',out_seg.shape)
        return out_seg, out_seg_1
...
class VNet(nn.Module):
    ...
    def forward(self, input):
        ...
        out_seg, out_seg_1 = self.decoder(features)
        features = self.pool(features[4])
        # print('---------------')
        # print('features.shape = ',features.shape)
        # print('out_seg.shape = ',out_seg.shape)
        # print('out_seg_1.shape = ',out_seg_1.shape)
        return out_seg, out_seg_1 # 4, 16, 112, 112, 80
