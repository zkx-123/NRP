...
def var_all_case_LA(model, num_classes, patch_size=(112, 112, 80), stride_xy=64, stride_z=64):
    # 120
    with open('/home/zkx/SSL4MIS-master/data/BM/' + '/{}'.format('val_40.txt'), 'r') as f:
        image_list = f.readlines()
    # image_list = ['/home/zkx/SSL4MIS-master/data/BM/' + "/images_120/{}.h5".format(
        # item.replace('\n', '').split(",")[0]) for item in image_list]
    # with open('/home/zkx/SSL4MIS-master/data/BM/' + '/{}'.format('val_crop.txt'), 'r') as f:
    #     image_list = f.readlines()
    image_list = ['/home/zkx/SSL4MIS-master/data/BM/' + "/h5_images/{}.h5".format(
        item.replace('\n', '').split(",")[0]) for item in image_list]
...
def test_all_case(model, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, save_result=True, test_save_path=None, preproc_fn=None, metric_detail=0, nms=0):
    ...
    total_metric = 0.0
    ith = 0
    individual_results = []  # 存储每个案例的结果
    total_metric = np.zeros(4)  # 初始化总指标（dice, jc, hd95, asd）
    for image_path in loader:
        # id = image_path.split('/')[-2]
        case_name = os.path.basename(image_path).split('.')[0]  # 从路径中提取案例名称
        h5f = h5py.File(image_path, 'r')
        image = h5f['image'][:]
        label = h5f['label'][:]
        ...
        # 记录个体结果
        individual_results.append([case_name, *single_metric])
    ...
    # 添加总体平均值行
    overall_row = ['Overall Average', *avg_metric]
    overall_df = pd.DataFrame([overall_row], columns=columns)
    
    # 合并数据并保存
    combined_df = pd.concat([individual_df, overall_df], ignore_index=True)
    excel_path = os.path.join(test_save_path, 'performance-18-4.xlsx')
    combined_df.to_excel(excel_path, index=False)
    
    with open(test_save_path+'performance-18-4.txt', 'w') as f:
...
def calculate_metric_percase(pred, gt):
    pred = np.array(pred, dtype=np.bool_)
    gt = np.array(gt, dtype=np.bool_)
